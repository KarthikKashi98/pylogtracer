...
class RootCauseAnalyzer:
    ...
    def analyze(self, extraction_result: Dict,
    ...
    def _get_chain(self):
        ...
        if self._chain is None:
            ...
            self._chain = (
                prompt
                | self.factory.get_llm()
                | StrOutputParser()
            )
        return self._chain

    def _call_llm(self, python_guess, freq_summary,
                  context_text, extra_context) -> str:
        extra_block = ""
        if extra_context:
            extra_block = (
    ...
    def _build_freq_summary(self, frequency: dict) -> str:
        ...
        return "\n".join(
            f"  - {etype}: {count} time(s)"
            for etype, count in list(frequency.items())[:5]
        )
